...
namespace apilogapi.Client
{
    public partial class ApiClient
    {
        ...
        partial void InterceptRequest(RestRequest request)
        {
            ...
            {
                // `/logs` エンドポイントのみに適用
                if (request.Resource == "/logs")
                {
                    // `created_date` のクエリパラメータを検索
                    var createdDateParam = request.Parameters
                        .FirstOrDefault(p => p.Type == ParameterType.QueryString && p.Name == "created_date");

                    if (createdDateParam != null && DateTimeOffset.TryParse(createdDateParam.Value.ToString(), out DateTimeOffset parsedDateOffset))
                    {
                        // パラメータを削除
                        request.RemoveParameter(createdDateParam);

                        // フォーマット済みの `created_date` パラメータを追加
                        request.AddQueryParameter("created_date", parsedDateOffset.ToString("yyyy-MM-dd"));
                    }
                }
